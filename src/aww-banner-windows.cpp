...
enum class NotificationPosition { TopLeft, TopRight, BottomLeft, BottomRight };
...
DesktopCoordinates getDesktopCoordinates(NotificationPosition position, int windowWidth,
                                         int windowHeight) {
  int x = 0;
  int y = 0;
...
static LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  ...
  case WM_CREATE: {
    ...
    NotificationDialogBox* my_class =
        reinterpret_cast<NotificationDialogBox*>(create_struct->lpCreateParams);
    SetWindowLongPtr(hwnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(my_class));
    std::cout << "Title: " << my_class->getTitle() << "\n";
  } break;
  case WM_PAINT: {
    NotificationDialogBox* my_class =
        reinterpret_cast<NotificationDialogBox*>(GetWindowLongPtr(hwnd, GWLP_USERDATA));
...
void NotificationDialogBox::show() {
  ...
  // Create a window
  const HWND hwnd = CreateWindowEx(WS_EX_CLIENTEDGE | WS_EX_LAYERED, "myWindowClass", title.c_str(),
  ...
  const COLORREF titleBg = RGB(titleBackgroundColor.get_red(), titleBackgroundColor.get_green(),
  ...
  const HFONT font =
      CreateFont(20, 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_OUTLINE_PRECIS,
