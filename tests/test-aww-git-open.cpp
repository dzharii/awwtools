...
class aww_git_open_io_dependencies_stub
    : public aww::internal::aww_git_open::aww_git_open_io_dependencies_interface {
public:
  // fs_get_absolute_path_stub
  std::function<std::filesystem::path(int, const std::filesystem::path&)>
      fs_get_absolute_path_stub =
          [this](int callCount, const std::filesystem::path& path) -> std::filesystem::path {
    return path;
  };
  ...
  std::function<aww::Result(int, std::filesystem::path&)>
      fs_get_current_directory_absolute_path_stub =
          [this](int callCount, std::filesystem::path& result) -> aww::Result {
    result = "/";
  ...
  aww::Result fs_get_current_directory_absolute_path(std::filesystem::path& result) override {
    ...
    return fs_get_current_directory_absolute_path_stub(
        fs_get_current_directory_absolute_path_called, result);
  }
  ...
  std::function<bool(int, const std::filesystem::path&)> fs_is_directory_stub =
  ...
  std::function<aww::Result(const std::string&)> launch_file_in_browser_stub =
      [this](const std::string& url) -> aww::Result { return aww::Result::ok(); };
...
TEST_CASE("try_convert_to_git_url accepatance tests") {
  ...
  const std::string input_url_SSH_azure_devops =
      "example@vs-ssh.visualstudio.com:v3/example/ProjectName/RepoName";
  const std::string input_url_HTTPS_azure_devops =
      "https://example.visualstudio.com/DefaultCollection/ProjectName/_git/RepoName";

  SUBCASE("try_convert_to_git_url: input_url_SSH_github") {
    std::string httpUrl;
    bool result =
        aww::internal::aww_git_open::try_convert_to_git_url(input_url_SSH_github, httpUrl);
    CHECK(result == true);
  ...
  SUBCASE("try_convert_to_git_url: input_url_HTTPS_github") {
    std::string httpUrl;
    bool result =
        aww::internal::aww_git_open::try_convert_to_git_url(input_url_HTTPS_github, httpUrl);
    CHECK(result == true);
  ...
  SUBCASE("try_convert_to_git_url: input_url_SSH_azure_devops") {
    std::string httpUrl;
    bool result =
        aww::internal::aww_git_open::try_convert_to_git_url(input_url_SSH_azure_devops, httpUrl);
    CHECK(result == true);
    CHECK(httpUrl ==
          "https://example.visualstudio.com/DefaultCollection/ProjectName/_git/RepoName");
  }
  ...
  SUBCASE("try_convert_to_git_url: input_url_HTTPS_azure_devops") {
    std::string httpUrl;
    bool result =
        aww::internal::aww_git_open::try_convert_to_git_url(input_url_HTTPS_azure_devops, httpUrl);
    CHECK(result == true);
    CHECK(httpUrl ==
          "https://example.visualstudio.com/DefaultCollection/ProjectName/_git/RepoName");
  }
...
TEST_CASE("try_find_repository_url_in_git_config accepatance tests") {
  aww_git_open_io_dependencies_stub deps;
  std::vector<std::string> gitConfigLines = {"[remote \"origin\"]",
                                             "    url = https://github.com/user/repo.git"};

  std::string gitSshOrHttpUri;
  aww::Result result = aww::internal::aww_git_open::try_find_repository_url_in_git_config(
      gitConfigLines, gitSshOrHttpUri);
  CHECK(result.is_ok());
  CHECK(result.error() == "");
...
TEST_CASE("find_git_repo accepatance tests") {
  ...
  SUBCASE("when executed from a valid nested folder of git repo, it should return the root git "
          "repo path") {
  ...
  SUBCASE("when executed from a valid root folder of git repo, it should return the root git repo "
          "path") {
    const std::filesystem::path root = "/";
...
TEST_CASE("get_relative_url_path acceptance tests") {
  ...
  std::string result;
  aww::Result res =
      aww::internal::aww_git_open::get_relative_url_path(parentAbsPath, childAbsPath, result);
  CHECK(res.is_ok());
